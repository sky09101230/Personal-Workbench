...
| 3a | `review-literature-metadata-evidence`: source updates become evidence-linked proposals; explicit current metadata confirmation; completeness and review state remain separate | Additive backed-up schema, dry-run/replay, no canonical value backfill; metadata/API/browser acceptance on disposable data. |
| 3b | Identity conflict resolution, explicit version evidence and Radar handoff validation; integrate remaining provenance and asset-state review | Preserve canonical IDs and evidence; no weak merge, ownership theft or silent preprint promotion. Independent tests, browser acceptance, validation and commit after 3a. |
...
- [x] Change 5 operational readiness accepted: `close-literature-foundation-operations`; 269 backend tests, frontend build, final isolated/browser checks. Current remaining gap is explicit, not hidden: 38 unavailable source PDFs (21 papers without owned primary), eight unresolved-parent descriptors and no supplied recovery backup.
- [x] Goal completion audit and OpenSpec main-spec synchronization complete. Missing source bytes remain explicit, recoverable operational exceptions; they are not represented as owned or scientifically verified.
