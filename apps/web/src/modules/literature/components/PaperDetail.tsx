...
export function PaperDetail({ paperId, onBack, onChanged }: { paperId: string; onBack: () => void; onChanged: () => void }) {
  ...
  return <article className="wb-detail"><button onClick={onBack}>← 返回文献库</button>
    ...
    {loading && !detail ? <p role="status">正在加载正式文献…</p> : detail && <>
      <header className="wb-detail-heading"><span className="wb-eyebrow">正式文献</span><h2>{detail.paper.title}</h2><p>{detail.paper.authors.join(", ") || "未记录作者"}</p><small>{[detail.paper.journal, detail.paper.year].filter(Boolean).join(" · ")}</small><div className="wb-actions">{detail.pdf_available && detail.paper.pdf_state !== "needs_recovery" && <a href={`/literature/papers/${encodeURIComponent(detail.paper.id)}/reader`}>{detail.paper.pdf_state === "source_only" ? "尝试读取来源 PDF" : "阅读 PDF"}</a>}<span className="wb-badge">元数据：{literatureLabel(detail.paper.metadata_status)}</span></div></header>
      ...
      {tab === "文件" && <section><h3>文件与本地资产</h3><p>本地文件不依赖 Zotero 连接器，可独立访问。</p>{!files.length && <p>暂无文件。可在下方添加 PDF。</p>}{files.map((file) => <article className="wb-review-item" key={file.id}><h4>{file.filename}</h4><p>{literatureLabel(file.role)} · {file.storage_kind === "zotero" ? "Zotero 来源引用（不代表文件可下载）" : "Workbench 管理的文件"} · {file.active && file.storage_kind === "zotero" ? "副本状态见上方校验" : literatureLabel(file.active ? file.availability : "detached")}</p>{file.storage_kind === "zotero" && file.active && file.downloadable && <button disabled={busy} onClick={() => void act(async () => { const result = await postJson<WorkflowResult>(`${base}/assets/${encodeURIComponent(file.id)}/acquire`); setMessage(`${file.filename} · ${literatureLabel(result.status)}${result.error ? ` · ${literatureLabel(result.error)}` : ""}`); refresh(); })}>获取 / 重试这个源 PDF</button>}{file.downloadable && !(file.storage_kind === "zotero" && detail.paper.pdf_state === "needs_recovery") && <div className="wb-actions"><a target="_blank" rel="noreferrer" href={`${base}/pdf?asset_id=${encodeURIComponent(file.id)}`}>{file.storage_kind === "zotero" ? "尝试打开来源文件" : "打开文件"}</a><a href={`${base}/pdf/download?asset_id=${encodeURIComponent(file.id)}`}>{file.storage_kind === "zotero" ? "尝试下载来源文件" : "下载"}</a></div>}</article>)}<button disabled={busy} onClick={() => void act(async () => { const result = await postJson<WorkflowResult>(`${base}/materialize-pdf`); setMessage(`${literatureLabel(result.status)}${result.error ? ` · ${result.error}` : ""}`); refresh(); })}>将 Zotero 主 PDF 保存到本地</button>{message && <p role="status">{message}</p>}<ImportPanel paperId={detail.paper.id} providerReady={false} syncing={false} onSync={() => undefined} onImported={refresh} /></section>}
