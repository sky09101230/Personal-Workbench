...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix='metadata-acceptance-', dir='.venv/tmp') as directory:
        ...
        print(json.dumps({'real_source_unchanged': True, 'dry_run_source_unchanged': True, 'historical_tables_checked': len(before), 'workflow_schema_version': dry['workflow_schema_version'], 'upgrade_replay_unchanged': True, 'source_overwrite_blocked': True, 'proposal_review_and_confirmation': True, 'canonical_and_research_state_preserved': True, 'integrity': 'ok'}, indent=2))
